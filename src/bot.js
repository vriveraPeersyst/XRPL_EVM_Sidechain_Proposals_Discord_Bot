...
async function cleanupEmptyProposalThreads(client) {
  ...
  for (const propId of Object.keys(scrapedProposals)) {
    ...
    if (hasScrapedVotes || hasKnownVotes) {
      ...
      // If thread exists, check its messages
      const thread = channel.threads.cache.get(threadId);
      if (!thread) {
        // Possibly the thread was deleted or not cached => remove votes
        console.log(
...
client.once('ready', async () => {
  ...
  async function executeTasks() {
    ...
    try {
      ...
      // 2) Validate proposals (detect new proposals, new votes, status changes, etc.)
      ...
      // 3) Persist combined data
      Object.assign(knownProposals, scrapedProposals);
      saveKnownProposals(knownProposals);

      // 4) Cleanup: Remove votes if thread is empty
